...
describe("makeAddressDummy", () => {
  describe("インスタンスが生成できる", () => {
    ...
    it("指定をする場合", () => {
      ...
      const postalCode = new PostalCode({ value: "3309301" });
      const prefecture = new Prefecture({ value: "埼玉県" });
      const city = "さいたま市浦和区";
      const town = "高砂３丁目１５−１";
      const block = "一階";
      const actual = makeFakeAddress({ id, accountId, postalCode, prefecture, city, town, block });
      expect(actual).toStrictEqual(expect.any(Address));
      expect(actual.id).toStrictEqual(expect.any(AddressId));
      ...
      expect(actual.postalCode).toStrictEqual(postalCode);
